...
export class IndexRequest implements ISortRequest, IPaginateRequest {
	@IsOptional()
	@IsString()
	@ApiProperty({ example: 'updatedAt', description: 'Sort entity by column name' })
	sort?: string;

	@IsOptional()
	@IsEnum(OrderDirectionEnum)
	@ApiProperty({ example: OrderDirectionEnum.Desc, description: `${OrderDirectionEnum.Asc} || ${OrderDirectionEnum.Desc}` })
